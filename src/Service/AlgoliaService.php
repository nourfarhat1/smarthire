...
class AlgoliaService
{
    ...
    /**
     * Index a single job offer using HTTP client
     */
    public function indexJob(JobOffer $job): bool
    {
        try {
            if (!$this->isAlgoliaConfigured()) {
                error_log("Algolia not configured, skipping job indexing");
                return false;
            }
            ...
            // Use HTTP client to index the job
            $response = $this->sendToAlgolia('/1/indexes/job_offers/' . $job->getId(), 'PUT', $jobData);
            ...
            error_log("Failed to index job to Algolia: " . $e->getMessage());
            return false;
        }
    }

    /**
     * Index all existing jobs using HTTP client
     */
    public function indexAllJobs(): int
    {
        try {
            ...
            foreach ($jobs as $job) {
                ...
                // Index each job individually using HTTP client
                $response = $this->sendToAlgolia('/1/indexes/job_offers/' . $job->getId(), 'PUT', $jobData);
            ...
            error_log("Indexed {$indexedCount} jobs to Algolia using HTTP client");
            return $indexedCount;
