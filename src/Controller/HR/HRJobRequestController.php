...
class HRJobRequestController extends AbstractController
{
    ...
    #[Route('/accept/{id}', name: 'app_hr_job_request_accept', methods: ['POST'])]
    public function acceptJobRequest(int $id): Response
    {
        $hrUser = $this->getUser();
        ...
        // Verify the job offer belongs to this HR user
        $jobOffer = $jobRequest->getJobOffer();
        if (!$jobOffer || $jobOffer->getRecruiter() !== $hrUser) {
            throw $this->createAccessDeniedException('You can only manage applications for your own job offers');
        }
        ...
        return $this->redirectToRoute('app_hr_job_requests');
    }

    #[Route('/reject/{id}', name: 'app_hr_job_request_reject', methods: ['POST'])]
    public function rejectJobRequest(int $id): Response
    {
        $hrUser = $this->getUser();
        $jobRequest = $this->jobRequestRepository->find($id);
