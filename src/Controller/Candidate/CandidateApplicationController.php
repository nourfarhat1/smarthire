...
class CandidateApplicationController extends AbstractController
{
    ...
    public function jobMarketplace(Request $request): Response
    {
        ...
        $savedJobIds = array_map(fn($job) => $job->getId(), $savedJobs->toArray());
    ...
    public function saveJob(Request $request, int $id, JobOfferRepository $jobOfferRepository): Response
    {
        ...
        // Save the job to user's saved jobs list
        try {
            $this->applicationService->saveJob($user->getId(), $jobOffer->getId());
            $this->addFlash('success', 'Job saved to your favorites!');
        } catch (\Exception $e) {
            $this->addFlash('error', 'Error saving job: ' . $e->getMessage());
        }

        return $this->redirectToRoute('app_candidate_job_marketplace');
    }

    #[Route('/unsave-job/{id}', name: 'app_candidate_unsave_job', methods: ['POST'])]
    public function unsaveJob(Request $request, int $id, JobOfferRepository $jobOfferRepository): Response
    {
        $user = $this->getUser();
        ...
        // Remove the job from user's saved jobs list
        try {
            $this->applicationService->unsaveJob($user->getId(), $jobOffer->getId());
            $this->addFlash('success', 'Job removed from your favorites!');
        } catch (\Exception $e) {
            $this->addFlash('error', 'Error removing job: ' . $e->getMessage());
        }
